...
function setPageScrollHeight(){
	...
	if(iframes.length==1){
		$('#mainContentIframe', window.parent.document).height($('body', window.parent.document).height()-50);
		//当前页面最外层的body
		var thisIframeBody=$(iframes.contents()).find('body');
		//导航条高度
		var narbarHeight=$(".crumbs").height();
		//正常页面的高度自适应
		$('.mainContent').css("height",thisIframeBody.height()-37);
		
		//页面中存在tab标签页重新计算tab标签页高度
		if($(thisIframeBody).find('.tabbable-custom').length!=0){
			  
			  $(thisIframeBody).find('.tab-content').css("height",$(thisIframeBody).find('.tabbable-custom').height()-52);
		}
		
		//$(thisIframeBody).find('#treeDemo2').height($(thisIframeBody).find(".tabbable-custom").height());
